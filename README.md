...
- 左侧 **探索案例** 回到第一组案例并启动轮播；综合测评由后端接口 `/api/comprehensive-question` 逐题自适应出题（worker 复用与测试同源的纯算法引擎与同一份题库），并按六维评分模型实时估计能力：作答期间仅展示已有作答维度的实时分数，通关后生成六维雷达、综合分与等级的智核觉醒报告。**Features** 恢复第一组案例。
...
- 顶部按最新参考图换为圆润的 **TEST!** 字标：`TestWordmark.jsx` 直接保留 `test-wordmark-reference.png` 的字形并去除深色底，不使用近似字体。四张卡片的中英文名称仍原样保留。卡片可以点击/键盘选择，出现选中标记；综合测评每关五题，根据前序作答逐题调整难度，同时兼顾六维覆盖、题型变化和防重复。每次综合测评是一条完整 Attempt：答题证据实时进入 `vendor/aiquos-six-dimension-scoring`（未改动的官方评分核心，版本 1.0.0）计算六维估计，进行中不显示总分与等级；答完 25 题且六维均有证据时判定完成，生成不可变历史快照并自动进入智核觉醒报告。测评页 **返回选择** 回到 CHOOSE! 页。
...
`/admin.html`（本地演示口令 `admin`）与学生端同设计体系：数据概览、学员管理、题库管理（编辑保存即发布新题库版本，学生端下一次出题生效）、测评记录与系统设置。概览学员为确定性演示班级，本机学员为这台机器上的真实作答；题库覆盖持久化在 `worker/bank-overrides.json`（已 gitignore），可随时恢复内置题库。
...
- `src/ForumBoard.jsx`、`src/forum-gallery.jsx`、`src/forum-board.css`：悬浮案例沙龙画廊论坛，集纳全部真实案例、核心 AI 提示词（支持一键复制）、发布者配文复盘、多分类筛选、用户发帖与 Home ↔ Cases ↔ Forum 全向平滑滑动转场。
